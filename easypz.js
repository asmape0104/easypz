...
exports.EasyPZLoader = EasyPZLoader;
...
exports.EzEventEmitter = EzEventEmitter;
...
exports.EasyPzZoomData = EasyPzZoomData;
...
exports.EasyPzCallbackData = EasyPzCallbackData;
...
var EasyPZ = /** @class */ (function () {
    ...
    EasyPZ.prototype.ensureTransformWithinBounds = function (transformBeforeScale) {
        if (this.options.bounds) {
            ...
            var scaleTopLeft = -1 * scale; //-1 * Math.max(scale, 0);
            var scaleBotRight = 0; //-1 * Math.min(scale, 0);
    ...
    var mode = {
        ...
        onMove: function (eventData) {
            if (mode.data.posStart1 && mode.data.posStart2) {
                ...
                var c = (posStart2.y - posStart1.y) / (posStart2.x - posStart1.x);
                var d = (posStart2.x * posStart1.y - posStart1.x * posStart2.y) / (posStart2.x - posStart1[0]);
                if (a !== c) {
                    var x = (d - b) / (a - c);
                    var y = (a * d - b * c) / (a - c);
                    var deg = (-Math.atan2(posStart2.y - posStart1.y, posStart2.x - posStart1.x) + Math.atan2(pos2.y - pos1.y, pos2.x - pos1.x)) * 180 / Math.PI;
                    easypz.onRotated.emit({ deg: deg, cx: x, cy: y });
